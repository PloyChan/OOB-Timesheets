...
//    }
    ...
    @HxRequest
    @PutMapping("/projects/{projectId}/{date}")
    public HtmxResponse updateTimeRegistration(@PathVariable int projectId, @PathVariable LocalDate date,
                                               Double value, Model model, Locale locale) {
        Duration duration = value == null ? Duration.ZERO : Duration.ofMinutes((long) (value * 60.0));
        timeRegistrationService.addOrUpdateRegistration(projectId, date, duration);
        ...
        return new HtmxResponse()
                .addTemplate("index :: #overall-total")
                .addTemplate("index :: day-total");
